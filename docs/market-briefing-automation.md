...
- `.github/workflows/publish-market-briefing.yml`
  ...
  - Node 22, Python 3.12로 생성기를 실행한다. KRX 자격증명이 있을 때 직접 수집용 `pykrx==1.2.8`을 설치한다.
...
- `scripts/fetch_investor_flows.py`
  - 인증된 `pykrx/KRX`를 1차로, 네이버 금융의 KRX 재배포 표를 2차로 사용하는 투자자별 수급 수집 헬퍼.
...
`KRX_ID`, `KRX_PW`는 pykrx/KRX 1차 경로에 사용한다. 직접 KRX 호출이 실패할 때만 자격증명 없이 공개되는 네이버 금융 `investorDealTrendDay`의 KRX 제공 표로 폴백한다. 수급 수집이 실패해도 전체 발행은 실패시키지 않는다. 장시작은 뉴스 기반 관전 포인트를 사용할 수 있지만, 장마감은 당일 정형 수급이 완전하지 않으면 수급 섹션을 생략한다.
...
뉴스는 특정 종목 예외 규칙이 아니라 공통 `marketEventSignals`로 해석한다.

- 시장·사건 전용·업종·특징주·수급·공시·일정 뉴스 후보를 함께 입력한다.
- 직접 가격 신호는 18시간, 구조적 사건은 24시간 안의 기사만 사건으로 승격한다.
- 각 사건에 영향 대상, 상방·하방·혼조 방향, 시장·업종·요인·개별 종목 범위, 강도, 복수 출처 확인 수를 기록한다.
- 환율, 금리, 유가, 관세, 지정학, 해외 증시와 주요 국내 업종을 같은 방식으로 평가한다.
- 업종 사건을 전체 지수로 확대하거나 개별 종목 사건을 업종 전체로 일반화하지 않는다.
- 시장에는 부담이지만 특정 업종에는 수혜인 사건은 `mixed`로 보존한다.
- 단일 개별 종목 사건은 사건 목록에 남기되 시장 방향 교정에는 사용하지 않는다.
- 상위 12개 사건만 writer 입력과 결정론적 사후 교정에 사용한다.
- 질문형·전망형 제목은 직접 가격 신호로 보지 않고 신뢰도와 우선순위를 낮춘다.
- 같은 대상의 상방·하방·혼조 신호는 `marketEventConclusions`에서 직접 가격 움직임, 시의성, 복수 출처를 합산해 하나의 최종 방향으로 결정한다.
- 지배 신호와 점수 차이가 작으면 혼조로, 직접 가격 신호가 명확히 우세하면 해당 방향으로 분류하고 반대 근거는 `counterEvidence`에 보존한다.
- 최종 브리핑에서 같은 대상은 맑음·흐림·비 중 한 곳에만 노출한다.
...
- KRX 자격증명이 있으면 pykrx/KRX를 항상 먼저 호출한다. 시장별 직접 KRX 호출이 실패할 때만 네이버 금융 KRX 표를 보조 호출한다.
