...
test('generator renders separate-page realtime navigation', () => {
  ...
  includes(source, "params.get('source') === 'maedo-signal'");
  includes(source, "/\\\\bwv\\\\b|MaedoSignal|maedo-signal|com\\\\.maedo\\\\.signal/iu");
...
test('briefing entry pages route realtime button to a separate page', () => {
  for (const relativePath of ['index.html', 'report/index.html']) {
    ...
    includes(html, "window.prompt('리딩방 비밀번호를 입력하세요')");
...
test('realtime shell pages exist, are member-only, and route back to briefing', () => {
  ...
  includes(rootRealtime, "function requireReadingRoomAccess()");
  includes(rootRealtime, "window.prompt('리딩방 비밀번호를 입력하세요')");
  includes(rootRealtime, "params.get('source') === 'maedo-signal'");
  includes(rootRealtime, "/\\bwv\\b|MaedoSignal|maedo-signal|com\\.maedo\\.signal/iu");
  includes(rootRealtime, 'if (requireReadingRoomAccess())');
